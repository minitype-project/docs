...
頻繁な使用が見込まれる機能については，[組込みプラグイン](../builtin-plugins)として標準で提供されています．
...
```ts
import type { BlockExtender, BodyContext, Text } from "minitype";
import { extractInlineText, isText, p } from "minitype";
...
```ts
import type { Block, PreBlockExtender } from "minitype";
import { p } from "minitype";
...
```ts
import type { BlockTransformer, ContentContext } from "minitype";
...
```ts
import type { ContentContext, InlineExtender } from "minitype";
