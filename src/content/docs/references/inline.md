...
title: インライン要素
caution:
  - InlineOrExtender の型に示されるインラインのうち，InlineExtender 以外のすべてを記載する．
  - "`src/lib/inline-helper.ts` に定義されるすべてのインラインヘルパを記載する．"
...
指定した幅の水平ボックスを作成します．
タブを設定したり，コンテンツを指定幅に収めたり，リーダ（点線等）を作成したりする場合に使用します．
