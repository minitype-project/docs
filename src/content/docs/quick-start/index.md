...
- `Q(n)`：級（1Q = 0.25 mm）．`Q(13)` は 3.25 mm です．
- `H(n)`：歯（1H = 0.25 mm）．実質的に `Q` と同等の役割を果たします．
- `em(n)`：文字サイズに対する相対値．`em(1)` は 1 文字分です．
- `cm(n)`：センチメートル．`cm(2)` は 20 mm です．
...
- 強制改行（`ForceBreak`）：`fbr()` で改行を挿入します．両端揃えが指定されている場合，直前の行は両端揃えとなります．
